...
public class RobotContainer {
    ...
    private void configureBindings() {
        ...
        drivetrain.setDefaultCommand(
            ...
            drivetrain.applyRequest(() ->
                drive.withVelocityX(-(DriveConstants.adjustSpeed(joystick.getLeftY()))) // Drive forward with negative Y (forward)// speed was getting multipled by MaxSpeed.
                    .withVelocityY(-(DriveConstants.adjustSpeed(joystick.getLeftX()))) // Drive left with negative X (left)
    ...
    /*    joystick.a().whileTrue(drivetrain.applyRequest(() -> brake));
        ...
        joystick.rightBumper().whileTrue(m_PivotSubsystem.pivotCommand(PivotConstants.PivotSpeed)).onFalse(m_PivotSubsystem.pivotCommand(0));
        joystick.rightTrigger(.5).whileTrue(m_PivotSubsystem.pivotCommand(-PivotConstants.PivotSpeed)).onFalse(m_PivotSubsystem.pivotCommand(0));

        joystick.leftBumper().whileTrue(m_ShooterSubsystem.runShooter(ShooterConstants.ShooterSpeed)).onFalse(m_ShooterSubsystem.runShooter(0));
        joystick.leftTrigger(.5).whileTrue(m_ShooterSubsystem.runShooter(ShooterConstants.ShooterSpeed)).onFalse(m_ShooterSubsystem.runShooter(0));
